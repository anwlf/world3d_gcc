...
/****************************************************************************/
/*   Analytics MATH implementation                                          */
/*   A. Volkov, 1991, 12.2012                                               */
/****************************************************************************/
...
REAL_TYPE abs(vector v) {
    REAL_TYPE vv=0; 
    VO(vv+=(v.e[k]*v.e[k]));
    if (vv>0) return sqrt(vv);
    else return vv;}
vector ort(vector v) {
    if (abs(v)!=0) return v/abs(v); 
...
rotation::rotation(vector w) {
    vector el=w;
    REAL_TYPE d=el.normalization(),sd=sin(d),cd=cos(d),ecd=1-cd;
    if (d>0) {
	e[0][1]=-sd*el[2];
	e[1][0]=sd*el[2];
	e[1][2]=-sd*el[0];
	...
	e[2][0]=-sd*el[1];
	e[0][2]=sd*el[1];
	MO(e[i][j]=((i==j)?cd+ecd*el[i]*el[j]:ecd*el[i]*el[j]+e[i][j]));
   } else
	MO(e[i][j]=((i==j)?1:0));
}
